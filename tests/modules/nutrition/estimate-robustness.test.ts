...
import {
  ...
  saveCacheMap,
  type CacheMap,
} from '@modules/nutrition/estimate-cache';
...
describe('cache map helpers', () => {
  it('returns null on a miss', () => {
    expect(getCached({}, 'milanesa', 1000)).toBeNull();
  });

  it('returns a stored entry within TTL', () => {
    const map = putCached({}, 'milanesa', sampleResult, 1000);
    expect(getCached(map, 'milanesa', 1000 + 5000, 10000)).toEqual(sampleResult);
  });

  it('treats expired entries as a miss', () => {
    const map = putCached({}, 'milanesa', sampleResult, 1000);
    expect(getCached(map, 'milanesa', 1000 + 10001, 10000)).toBeNull();
  });

  it('evicts the oldest entries past the max size', () => {
    let map: CacheMap = {};
    for (let i = 0; i < 5; i++) {
      map = putCached(map, `food-${i}`, { ...sampleResult, calories: i }, 1000 + i, 3);
    }
    const keys = Object.keys(map);
    expect(keys.length).toBe(3);
    // Oldest two (food-0, food-1) evicted; newest three remain.
    expect(keys.sort()).toEqual(['food-2', 'food-3', 'food-4']);
  });

  it('does not mutate the input map (pure)', () => {
    const original: CacheMap = {};
    putCached(original, 'milanesa', sampleResult, 1000);
    expect(original).toEqual({});
  });
});

describe('cache localStorage adapter', () => {
  ...
  it('round-trips a cache map through storage', () => {
    const storage = fakeStorage();
    const map = putCached({}, 'milanesa con papas', sampleResult, 1000);
    saveCacheMap(map, storage);
    expect(loadCacheMap(storage)).toEqual(map);
  });

  it('returns an empty map when storage is empty or corrupt', () => {
    const storage = fakeStorage();
    expect(loadCacheMap(storage)).toEqual({});
    storage.setItem('hubtify_nutri_estimate_cache', '{not json');
    expect(loadCacheMap(storage)).toEqual({});
  });

  it('is a no-op when no storage is available', () => {
    expect(() => saveCacheMap({}, null)).not.toThrow();
    expect(loadCacheMap(null)).toEqual({});
  });
});
