...
describe('reopenDayRecord', () => {
  ...
  it('reopening soft-deletes the record, returns the granted XP/HP, and frees the guard', () => {
    ...
    // `closedAt` se sumó al retorno: la reversión del XP la hace el motor por
    // la vía de undo, y este handler ya solo reporta lo que había guardado.
    expect(result).toMatchObject({ xpTotal: 42, hpChange: 10 });
