...
describe('mergeNutritionData — daily_closed LWW', () => {
  ...
  it('replicates a remote reopen (newer record carrying deleted_at) as a soft delete', async () => {
    ...
    expect(row.deletedAt).toBe(NEW); // reopened across accounts
  });
...
describe('mergeNutritionData — food_log merge by id', () => {
  ...
  it('soft-deletes a local row when remote carries a newer deleted_at', async () => {
    ...
    await merge({ foodLog: [{ id: 100, date: '2026-05-01', time: '12:00', description: 'Lunch', calories: 500, source: 'manual', updated_at: NEW, deleted_at: NEW }] });
    expect(readFood(100).deletedAt).toBe(NEW);
  });

  it('does not soft-delete when the local row is already newer than the remote delete', async () => {
    insertLocalFood(100, 500, NEW, null);
    await merge({ foodLog: [{ id: 100, date: '2026-05-01', time: '12:00', description: 'Lunch', calories: 500, source: 'manual', updated_at: OLD, deleted_at: OLD }] });
